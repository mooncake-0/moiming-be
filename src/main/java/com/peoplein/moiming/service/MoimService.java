...
public class MoimService {
    ...
    // 모임 생성
    public String createMoim(MoimCreateReqDto requestDto, Member curMember) {
        ...
        List<MoimCategoryLinker> moimCategoryLinkers = categories.stream()
                .map(category -> new MoimCategoryLinker(moim, category))
    ...
    private List<Category> generateCategoryList(List<String> categoryNameValues) {
        List<CategoryName> queryList = categoryNameValues.stream().map(CategoryName::valueOf).collect(Collectors.toList());
