...
import com.peoplein.moiming.service.MoimServiceOld;
...
@RequestMapping(NetworkSetting.API_SERVER + "/v1" + NetworkSetting.API_MOIM)
public class MoimControllerOld {

    private final MoimServiceOld moimServiceOld;
    ...
    public ResponseEntity<?> createMoim(@RequestBody MoimRequestDto requestDto) {
        ...
        MoimResponseDto moimResponseDto = moimServiceOld.createMoim(curMember, requestDto);
    ...
    public ResponseEntity<?> viewMemberMoim() {
        Member curMember = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        List<MoimResponseDto> responseData = moimServiceOld.viewMemberMoim(curMember);
    ...
    public ResponseEntity<?> getMoim(@PathVariable(name = "moimId") Long moimId) {
        Member curMember = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        MoimResponseDto responseDto = moimServiceOld.getMoim(moimId, curMember);
    ...
    public ResponseEntity<?> updateMoim(@RequestBody MoimRequestDto moimRequestDto) {
        Member curMember = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        MoimResponseDto responseDto = moimServiceOld.updateMoim(moimRequestDto, curMember);
    ...
    public ResponseEntity<?> deleteMoim(@PathVariable(name = "moimId") Long moimId) {
        Member curMember = (Member) SecurityContextHolder.getContext().getAuthentication().getPrincipal();
        moimServiceOld.deleteMoim(moimId, curMember);
